...
class ArucoLocalizer():
    def __init__(
        ...
        for image_idx in self.photogrammetry_software.images.keys():
            self.image_names.append(
                self.photogrammetry_software._src_image_path.joinpath(
                    self.photogrammetry_software.images[image_idx].name
                ).__str__()
            )
    ...
    def __detect(self):
        ...
        for image_idx, image_key in enumerate(
            self.photogrammetry_software.images.keys()
        ):
            ratio_x = (
                self.photogrammetry_software.cameras[1].width / result[image_idx][2][1]
            )
            ratio_y = (
                self.photogrammetry_software.cameras[1].height / result[image_idx][2][0]
            )
            if result[image_idx][0] != None:
                ...
                self.photogrammetry_software.images[image_key].aruco_corners = result[
                    image_idx
                ][0]

            self.photogrammetry_software.images[image_key].aruco_id = result[image_idx][
                1
            ]
            self.photogrammetry_software.images[
                image_key
            ].image_path = self.image_names[image_idx]
    ...
    def __ray_cast(self):
        ...
        for image_idx in self.photogrammetry_software.images.keys():
            if self.photogrammetry_software.images[image_idx].aruco_corners is not None:
                current_id = self.photogrammetry_software.images[image_idx].aruco_id[
                    0, 0
                ]

                # Process all detected ArUco markers
                p0, n = ray_cast_aruco_corners(
                    extrinsics=self.photogrammetry_software.images[
                        image_idx
                    ].extrinsics,
                    intrinsics=self.photogrammetry_software.images[
                        image_idx
                    ].intrinsics.K,
                    corners=self.photogrammetry_software.images[
                        image_idx
                    ].aruco_corners,
                )
    ...
    def apply(self) -> Tuple[o3d.pybind.geometry.PointCloud, float]:
        ...
        self.photogrammetry_software.dense_scaled = deepcopy(
            self.photogrammetry_software.dense
        )
        self.photogrammetry_software.dense_scaled.scale(
            scale=self.scale_factor, center=np.asarray([0.0, 0.0, 0.0])
        )

        self.sparse_scaled = deepcopy(self.photogrammetry_software.sparse)
        for num in self.photogrammetry_software.sparse.keys():
            self.sparse_scaled[num].xyz = (
                self.sparse_scaled[num].xyz * self.scale_factor
            )

        # ToDo: Scale tvec and save
        self.photogrammetry_software.images_scaled = deepcopy(
            self.photogrammetry_software.images
        )
        for idx in self.photogrammetry_software.images_scaled.keys():
            self.photogrammetry_software.images_scaled[idx].tvec = (
                self.photogrammetry_software.images[idx].tvec * self.scale_factor
            )
    ...
    def calculate_all_aruco_positions(self):
        ...
        for aruco_id in self.all_aruco_ids:
            if (
                len(self.all_P0[aruco_id]) > 0
            ):  # Only process if we have data for this ID
    ...
    def write_data(self):
        ...
        sparse_scaled_path = self.photogrammetry_software._project_path.joinpath(
            "sparse_scaled"
        )
        ...
        o3d.io.write_point_cloud(
            os.path.join(pcd_scaled, "scaled.ply"),
            self.photogrammetry_software.dense_scaled,
        )

        # Save scale factor
        scale_factor_file_name = self.photogrammetry_software._project_path.joinpath(
            "sparse_scaled/scale_factor.txt"
        )
