...
from ..markers.aruco import localize_markers, localize_apriltag_markers
...
Marker = collections.namedtuple(
    "Marker",
    [
        "id",  # Marker ID (ArUco marker ID or AprilTag ID)
        "xyz",  # Center position in 3D
        "corners_3d",  # 4x3 array of corner positions in 3D
        "type",  # marker type: 'aruco' or 'apriltag'
        "tag_family",  # ArUco dictionary type (int) or AprilTag family (str)
...
class SfmProjectBase(ABC):
    ...
    def __init__(self, project_path):
        ...
        self._images = {}
        self._points3D = {}
        self._markers = {}  # Structure: {dict_type_or_family: {marker_id: Marker}}
        self._load_data()
    ...
    def detect_markers(
        ...
        logging.info(
            f"ArUco detection complete. Found {len(results)} markers for type {dict_type}."
        )
        return results
    ...
    def detect_apriltag_markers(
        ...
        :param tag_family: AprilTag family to use (e.g., "tag36h11")
        :param progress_bar: Show progress bars
        ...
        raw_results = localize_apriltag_markers(
            project=self,
        ...
        # Store results in project using Marker namedtuples
        self._store_apriltag_results(tag_family, raw_results)

        logging.info(
            f"AprilTag detection complete. Found {len(raw_results)} markers for family {tag_family}."
        )
        return raw_results

    def _store_marker_results(self, dict_type: int, raw_results: Dict):
        """
        Store ArUco marker results using Marker namedtuples with 3D corners included.
        ...
        for aruco_id, data in raw_results.items():
            ...
            logging.info(f"Stored ArUco marker dict={dict_type}, id={aruco_id}")

    def _store_apriltag_results(self, tag_family: str, raw_results: Dict):
        """
        Store AprilTag marker results using Marker namedtuples with 3D corners included.

        :param tag_family: AprilTag family
        :param raw_results: Raw results from localize_apriltag_markers function
        """
        # Initialize storage if needed
        if not hasattr(self, "_markers"):
            self._markers = {}

        # Initialize for this tag family
        if tag_family not in self._markers:
            self._markers[tag_family] = {}

        for apriltag_id, data in raw_results.items():
            # Create Marker namedtuple with 3D corners included
            marker = Marker(
                id=apriltag_id,
                xyz=data["center_xyz"],
            ...
            self._markers[tag_family][apriltag_id] = marker

            logging.info(
                f"Stored AprilTag marker family={tag_family}, id={apriltag_id}"
            )
    ...
    def _verify_data_loaded(self):
        ...
        print(
            f"Loaded {len(self._cameras)} cameras, {len(self._images)} images, {len(self._points3D)} points"
        )
            ...
            for dict_type in self._markers:
                for aruco_id in self._markers[dict_type]:
                    ...
                    self._markers[dict_type][aruco_id] = marker._replace(
                        xyz=new_center, corners_3d=transformed_corners_3d
                    )
